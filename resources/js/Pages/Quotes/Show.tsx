...
import { ArrowLeft, Printer, FileCheck, Edit2, Download, Building2, User2, Clock, CheckCircle2, FileText, Send, Calendar, Activity } from 'lucide-react';
import { Quote, Setting, PageProps, ActivityLog } from '@/types/models';
...
interface ShowProps {
    quote: Quote & { activity_logs?: ActivityLog[] };
}
...
export default function Show({ quote }: ShowProps) {
    ...
    const handleResend = () => {
        setIsResending(true);
        router.post(route('quotes.resend', quote.id), {}, {
            onFinish: () => setIsResending(false)
        });
    };

    const handlePublicUrl = () => {
        const url = route('quotes.public', quote.id);
                ...
                actions={
                    <>
                        ...
                        <a href={route('quotes.download', quote.id)}>
                            <Button variant="soft" icon={<Download className="w-4 h-4" />} className="bg-white border-slate-200">
                                PDF
                            </Button>
                        </a>
                        ...
                        <CardContent className="p-8 md:p-12 space-y-12">
                            ...
                            <div className="flex flex-col md:flex-row justify-between gap-12 pt-8">
                                <div className="flex-1 space-y-4">
                                    <div className="p-6 rounded-2xl bg-primary-50/30 border border-primary-100/50 flex items-start gap-4 max-w-md">
                                        <CheckCircle2 className="w-5 h-5 text-primary-600 shrink-0 mt-0.5" />
                                        <div className="space-y-1">
                                            <p className="text-xs font-black text-primary-900 uppercase tracking-widest">Estimation Verified</p>
                                            <p className="text-[10px] text-primary-700 leading-relaxed font-semibold">
