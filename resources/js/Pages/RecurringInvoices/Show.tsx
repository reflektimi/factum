...
import { RecurringInvoice, Setting, PageProps } from '@/types/models';
...
export default function Show({ recurringInvoice }: ShowProps) {
    ...
    return (
            ...
            header={
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <Link href={route('recurring-invoices.index')} className="text-slate-500 hover:text-slate-700 print:hidden">
                            <ArrowLeft className="w-6 h-6" />
                        </Link>
                        <h2 className="text-xl font-semibold leading-tight text-slate-800 font-heading">
                            Recurring Profile: {recurringInvoice.profile_name}
                        </h2>
                        <StatusBadge status={recurringInvoice.status} className="ml-2" />
                    </div>
        ...
        >
            <Head title={`Recurring Invoice - ${recurringInvoice.profile_name}`} />
                    ...
                    <Card className="print:shadow-none print:border-none">
                        <CardContent className="p-8 space-y-8">
                            ...
                            <div className="flex justify-between items-start border-b border-slate-100 pb-8">
                                <div>
                                     {settings?.logo_path ? (
                                        ...
                                        <h1 className="text-2xl font-bold text-slate-900" style={{ color: primaryColor }}>RECURRING INVOICE</h1>
                                    )}
                                    <p className="text-slate-500 mt-1 uppercase tracking-wider text-sm font-semibold">{recurringInvoice.interval}</p>
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-8">
                                <div>
                                    <h4 className="text-sm font-medium text-slate-500 uppercase tracking-wider mb-2">Billed To</h4>
                                    <p className="font-semibold text-slate-900">{recurringInvoice.customer?.name}</p>
                                    <p className="text-slate-600 text-sm">{recurringInvoice.customer?.contact_info?.email}</p>
                                    <p className="text-slate-600 text-sm">{recurringInvoice.customer?.contact_info?.address}</p>
                                </div>
                                <div className="text-right">
                                    <div className="space-y-2">
                                        <div className="flex justify-between md:justify-end md:gap-8">
                                            <span className="text-slate-500">Start Date:</span>
                                            <span className="font-medium text-slate-900">{formatDate(recurringInvoice.start_date)}</span>
                                        </div>
                                        <div className="flex justify-between md:justify-end md:gap-8">
                                            <span className="text-slate-500">Next Run:</span>
                                            <span className="font-medium text-slate-900">{recurringInvoice.next_run_date ? formatDate(recurringInvoice.next_run_date) : '-'}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="border-t border-slate-100 pt-8 flex justify-end">
                                <div className="w-64 space-y-3">
                                    <div className="flex justify-between text-lg font-bold text-slate-900 border-t border-slate-200 pt-3" style={{ color: primaryColor }}>
                                        <span>Total</span>
                                        <span>{formatCurrency(recurringInvoice.total_amount)}</span>
                                    </div>
                                </div>
                            </div>
