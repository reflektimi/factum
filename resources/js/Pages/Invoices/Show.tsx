...
import { Invoice, Setting, PageProps, ActivityLog } from '@/types/models';
...
export default function Show({ invoice }: ShowProps) {
    ...
    const handleResend = () => {
        setIsResending(true);
        router.post(route('invoices.resend', invoice.id), {}, {
            onFinish: () => setIsResending(false)
        });
    };

    const handlePublicUrl = () => {
        const url = route('invoices.public', invoice.id);
                ...
                <PageHeader 
                    ...
                    actions={
                        <div className="flex gap-3 print:hidden">
                            ...
                            <Button 
                                variant="primary" 
                                onClick={() => window.location.href = route('invoices.download', invoice.id)}
                                ...
                                <div className="p-8 md:p-12 space-y-12">
                                        ...
                                        <Table>
                                            <TableHeader className="bg-slate-50/80 border-b border-slate-100">
                                                <TableRow className="hover:bg-transparent">
                                                    ...
                                                    <TableHead className="py-5 text-slate-900 font-black text-[10px] uppercase tracking-widest" align="right">Unit Price</TableHead>
                                                    <TableHead className="py-5 text-slate-900 font-black text-[10px] uppercase tracking-widest pr-6" align="right">Amount</TableHead>
                                                </TableRow>
                                            </TableHeader>
                                            <TableBody>
                                                {items.map((item, index) => (
                                                    <TableRow key={index} className="hover:bg-slate-50/30 border-slate-50 group">
                                                        ...
                                                        <TableCell className="py-6 pr-6 text-slate-900 font-black font-mono text-sm" align="right">
                                    ...
                                    <div className="flex flex-col md:flex-row gap-12 justify-between items-end pt-8">
                                        <div className="w-full max-w-[280px]">
                                            <div className="p-4 bg-primary-50/30 rounded-2xl border border-primary-100/50 flex items-center gap-4 group">
                                                <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center text-primary-600 shadow-sm group-hover:scale-110 transition-transform">
                                                    <CheckCircle2 className="w-5 h-5" />
                                                </div>
                                                <div className="text-[10px] leading-relaxed font-bold text-primary-700 uppercase tracking-tight">
                    ...
                    <div className="lg:col-span-4 space-y-6 print:hidden">
                        ...
                        <Card className="border-none shadow-premium-soft bg-slate-900 text-white overflow-hidden relative">
                            ...
                            <CardContent className="p-8 space-y-6 relative z-10">
                                ...
                                <div className="pt-2">
                                    <Button 
                                        className="bg-primary-600 hover:bg-primary-500 text-white border-none font-bold h-11 text-xs uppercase tracking-widest shadow-lg shadow-primary-900/20 transition-all hover:scale-[1.02] active:scale-[0.98]" 
                                        fullWidth
                                        onClick={() => router.visit(route('payments.create', { invoice_id: invoice.id }))}
