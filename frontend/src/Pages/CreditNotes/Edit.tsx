...
export default function Edit() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [noteRes, customersRes] = await Promise.all([
                    api.get(`/api/credit-notes/${id}`),
                    api.get('/api/accounts?type=customer&per_page=100')
                ]);
                
                const note = noteRes.data.creditNote;
                const initialItems = (typeof note.items === 'string' 
                ...
                setData({
                    ...
                    status: note.status,
                    notes: note.notes || '',
                    items: initialItems.length > 0 ? initialItems : [{ description: '', quantity: 1, price: 0, total: 0 }],
                    total_amount: note.amount,
                });
                
                setCustomers(customersRes.data.accounts?.data || customersRes.data.accounts || []);
            } catch (error) {
                console.error('Failed to fetch credit note edit data:', error);
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSubmit: FormEventHandler = async (e) => {
        ...
        try {
            await api.put(`/api/credit-notes/${id}`, data);
        ...
        return (
            <AuthenticatedLayout>
                <div className="flex h-[60vh] items-center justify-center">
                    <div className="text-slate-500 animate-pulse font-medium">Retrieving financial ledger...</div>
                </div>
            </AuthenticatedLayout>
        );
    }
                ...
                title={
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-slate-400 font-mono text-lg font-medium">{data.number}</span>
                        </div>
                    </div>
                }
