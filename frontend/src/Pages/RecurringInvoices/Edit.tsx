...
import { ArrowLeft, Save, Plus, Trash2, FileText, Calculator, Calendar, Activity, CheckCircle2 } from 'lucide-react';
...
export default function Edit() {
        ...
        const fetchData = async () => {
            try {
                const [profileRes, customersRes] = await Promise.all([
                    ...
                    api.get('/api/accounts?type=customer&per_page=100')
                ]);
                
                const profile = profileRes.data.recurringInvoice;
                const items = (typeof profile.items === 'string' 
                    ? JSON.parse(profile.items) 
                    : profile.items) as InvoiceItem[];

                setData({
                    profile_name: profile.profile_name,
                    customer_id: profile.customer_id,
                    interval: profile.interval,
                    start_date: profile.start_date.split('T')[0],
                    status: profile.status,
                    auto_send: profile.auto_send,
                    items: items.length > 0 ? items : [{ description: '', quantity: 1, price: 0 }],
                });
    ...
    const handleSubmit: FormEventHandler = async (e) => {
        ...
        try {
            await api.put(`/api/recurring-invoices/${id}`, data);
        ...
        return (
            <AuthenticatedLayout>
                <div className="flex h-[60vh] items-center justify-center">
                    <div className="text-slate-500 animate-pulse font-medium">Synchronising profile parameters...</div>
                </div>
            </AuthenticatedLayout>
        );
    }
            ...
            <PageHeader 
                title={
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-3">
                            <span>Edit Recurring Profile</span>
                            <span className="text-slate-400 font-mono text-lg font-medium">{data.profile_name}</span>
                        </div>
                    </div>
                }
                subtitle="Modify automated billing parameters and lifecycle status"
            />
            <Head title={`Edit ${data.profile_name}`} />
                                ...
                                <CardContent className="p-6 space-y-6">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <Select
                                            ...
                                            onChange={(e) => setData({ ...data, customer_id: parseInt(e.target.value) })}
                                        ...
                                        >
                                            ...
                                            <option value="ended">Ended Cycle</option>
                                        </Select>
                                    </div>
                                </CardContent>
                            </Card>
                        ...
                        <div className="col-span-12 lg:col-span-4 relative">
                            <div className="lg:sticky lg:top-6 space-y-6">
                                <Card className="border-none shadow-premium-soft overflow-hidden group">
                                    ...
                                    <CardContent className="p-6 space-y-6">
                                        ...
                                        <p className="text-[10px] text-slate-400 text-center leading-relaxed font-medium px-4">
                                            Updates to recurring cycles are logged and will affect all future generated documents.
                                        </p>
                                    </CardContent>
                                </Card>
                                ...
                                <div className="mt-6 p-6 bg-slate-50 rounded-[2rem] border border-slate-100 border-dashed text-center">
                                    ...
                                    <p className="text-xs text-slate-500 leading-relaxed mb-4">Every change to this profile is logged for compliance and transparency.</p>
