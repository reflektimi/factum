...
import { Plus, Search, CreditCard, MoreVertical, Edit2, Trash2, ExternalLink, Wallet, Banknote, Loader2 } from 'lucide-react';
import type { PaginatedData, Payment } from '@/types/models';
...
export default function Payments() {
    ...
    const [payments, setPayments] = useState<PaginatedData<Payment> | null>(null);
    ...
    return (
        <AuthenticatedLayout>
                    ...
                    <div className="w-full md:w-64">
                        <Select
                            ...
                            onChange={(e) => {
                                ...
                                setMethod(newMethod);
                                const newParams = new URLSearchParams(location.search);
                                if (newMethod) newParams.set('method', newMethod); else newParams.delete('method');
                                navigate({ search: newParams.toString() }, { replace: true });
                            }}
                                        ...
                                        <TableActions>
                                            <Dropdown>
                                                ...
                                                <Dropdown.Content align="right" width="48">
                                                    <Dropdown.Link to={`/payments/${payment.id}`} className="flex items-center gap-2 text-sm">
                                                        <ExternalLink className="w-4 h-4 opacity-50" />
                                                        Trace Transaction
                                                    </Dropdown.Link>
                                                    <Dropdown.Link to={`/payments/${payment.id}/edit`} className="flex items-center gap-2 text-sm">
            ...
            {payments && <Pagination data={payments} />}
            ...
            <DeleteConfirmation
                ...
                onConfirm={async () => {
                    try {
                        await api.delete(`/api/payments/${deleteId}`);
                        setDeleteId(null);
                        fetchData();
                    } catch (error) {
